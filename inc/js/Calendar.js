...
function Calendar_D(obj)
{
    ...
    {
        Show_cal(nowDay.getFullYear(), nowDay.getMonth()+1, nowDay.getDate());
    }
}
...
function Calendar_M(obj)
{
    ...
    {
        Show_cal_M( nowMon.getFullYear(), nowMon.getMonth() + 1 );
    }
}
...
function Show_cal(sYear, sMonth, sDay)
{
    ...
    Cal_HTML += "<tr height='25' align=center bgcolor='#f4f4f4'>";
    ...
    for (intLoopWeek=1; intLoopWeek < 7; intLoopWeek++) // 요일단위 루프 시작, 일요일 부터
    {
        Cal_HTML += "<tr height='20' align=right bgcolor='white'>"
        for (intLoopDay=1; intLoopDay <= 7; intLoopDay++) // 요일단위 루프 시작, 일요일 부터
        {
            ...
            else // 입력 날짝 월말보다 크다면
            {
                ...
                { // 입력날짜가 현재월에 해당 되면
                    ...
                    switch(intLoopDay) // 일요일이면 빨간 색으로
                    {
                        case 1:
                            Cal_HTML += "color:red;"
                            break;
    ...
    Cal_HTML += "<tr height='20' align=center bgcolor='#f0f0f0'>";
    Cal_HTML += "<td colspan=7 align=center style='padding:5px' onClick=parent.Calendar_Click(this); title=" + nowDay.getFullYear() + "-" + day2( nowDay.getMonth() + 1 ).toString() + "-" + day2( nowDay.getDate() ).toString()+ " onFocus='this.blur()' style='cursor:Hand;border;text-decoration:none;color:black;'><span id='lblToday'>오늘 날짜 : <font color=blue>" + nowDay.getFullYear() + "년 " + ( nowDay.getMonth() + 1 ) + "월 " + nowDay.getDate() + "일</span></td>";
    Cal_HTML += "</tr>";
    ...
    //행이 6개 행인지, 5개인지 구분
    if (intLoopWeek == 6)   calHeight = 202;
    else                    calHeight = 182;
...
function Show_cal_M(sYear, sMonth)
{
    ...
    Cal_HTML += "</td></tr>\n";
    Cal_HTML += "<tr><td colspan=3 height='1' bgcolor='#000000'></td></tr>";
    ...
    oPopup.show(pop_left, (pop_top + target.offsetHeight), 160, 119, document.body);
}
